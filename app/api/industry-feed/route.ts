...
import { fetchSourceListingWithRecovery, type ListingFetchResult } from '@/lib/intelligence/source-failure-recovery';
import { prioritizeFreshCandidates, freshnessStateSummary } from '@/lib/intelligence/incremental-crawl';
import { buildAdaptiveSourcePlan, recordAdaptiveSourceOutcomes, adaptiveSourceStateSummary } from '@/lib/intelligence/adaptive-source-crawl';
...
export async function GET(req:NextRequest){
  ...
  const adaptiveSourcePlan=buildAdaptiveSourcePlan(enabledSources,new Date(fetchedAt));
  const planBySource=new Map(adaptiveSourcePlan.map(x=>[x.source.id,x] as const));
  const orderedSources=adaptiveSourcePlan.map(x=>x.source);
  const results=await mapWithConcurrency(orderedSources,8,async source=>{
    const recovered=await fetchSourceListingWithRecovery({
    ...
    return {source,items,error:null as string|null,recovery:recovered.diagnostics,freshness:fresh.diagnostics};
  });
  ...
  const enriched=await mapWithConcurrency(clusters,10,async group=>{
    ...
    try{const articleHtml=await fetchText(primary.url); article=extractArticle(articleHtml,profile.keywords); discoveryObservations.push({pageUrl:primary.url,pageSource:primary.source,html:articleHtml}); articleReadOkCount++; articleExtractionMethods[article.extractionMethod]=(articleExtractionMethods[article.extractionMethod]??0)+1; if(article.extractionMethod!=='none'&&article.extractionMethod!=='metadata')articleExtractionRecovered++; if(article.extractedChars<120)articleExtractionThin++; if(article.publishedAt)dateRecoveredFromArticle++;}catch{articleReadOk=false; articleReadFailed++;}
  ...
  const rotatingNewsQueue=rotatingNewsCandidates.slice(0,5);
  const coverageNewsQueue=profile.id==='waste'?buildCoverageDrivenNewsQueue({items,now:new Date(fetchedAt),maxQueries:3}):[];
  const sourceExpansionQueue=profile.id==='waste'?buildSourceExpansionQueue(new Date(fetchedAt),4):[];
  const targetNewsSlots=Math.min(12,newsIntakeProviderPolicy.maxQueriesPerRun);
  const usedNewsSlots=rotatingNewsQueue.length+coverageNewsQueue.length+sourceExpansionQueue.length;
  const newsBackfill=rotatingNewsCandidates.slice(5,5+Math.max(0,targetNewsSlots-usedNewsSlots));
  const newsDiscoveryQueue=[...rotatingNewsQueue,...coverageNewsQueue,...sourceExpansionQueue,...newsBackfill];
  const municipalProtocolQueue=profile.id==='waste'?buildMunicipalProtocolQueue(new Date(fetchedAt),3):[];
  const competitorJobQueue=profile.id==='waste'?buildCompetitorJobQueue(new Date(fetchedAt),3):[];
  ...
  const activeDiscovery=await runDiscoveryOrchestrator({providers:configuredProviders,queue:combinedDiscoveryQueue,industryKeywords:profile.keywords,knownUrls:items.map(x=>x.url),maxAgeDays:30,policy:newsIntakeProviderPolicy});
  ...
  const gapDrivenPolicy={...newsIntakeProviderPolicy,maxQueriesPerRun:4,maxResultsPerQuery:8,maxEstimatedCostPerRun:0.04};
  const gapDrivenDiscovery=await runDiscoveryOrchestrator({
    providers:configuredProviders,
    queue:rankedGapDrivenQueue,
  ...
  const competitorWebsiteMonitor=profile.id==='waste'?await monitorCompetitorWebsites(new Date(fetchedAt),6):{changes:[],diagnostics:{checked:0,failed:0,baselinesCreated:0,changesDetected:0,persistence:'process-local' as const,verifiedPages:0}};
  ...
  const sourceStatus=results.map(r=>{
    ...
    return {
      id:r.source.id,name:r.source.name,type:r.source.type,scope:r.source.scope,tier:r.source.tier,description:r.source.description??'',ok:!r.error,hits:r.items.length,error:r.error,recovery:r.recovery,
      value:evaluateSourceValue({ok:!r.error,hits:r.items.length,primaryItems,confirmationContributions})
    };
  });
  recordAdaptiveSourceOutcomes(sourceStatus.map(s=>({
    sourceId:s.id,
    ok:s.ok,
    candidates:s.hits,
    unseen:results.find(r=>r.source.id===s.id)?.freshness.unseen??0,
    acceptedPrimary:s.value.primaryItems,
    confirmations:s.value.confirmationContributions,
  ...
  return NextResponse.json({
    ...
    network:summarizeSourceNetwork(profile.sources),sourceDiversity,newsIntakeDiagnostics,newsQualitySummary,freshEventSummary,articleValidationSummary,bidNewsRelevanceSummary,newsIntake:{fixedSources:enabledSources.length,adaptiveSourceCrawl:{state:adaptiveSourceStateSummary(),priority:adaptiveSourcePlan.filter(x=>x.lane==='priority').length,standard:adaptiveSourcePlan.filter(x=>x.lane==='standard').length,explore:adaptiveSourcePlan.filter(x=>x.lane==='explore').length,plan:adaptiveSourcePlan.map(x=>({id:x.source.id,name:x.source.name,lane:x.lane,priorityScore:x.priorityScore,candidateLimit:x.candidateLimit,reasons:x.reasons}))},freshness:{state:freshnessStateSummary(),considered:results.reduce((n,x)=>n+x.freshness.considered,0),unseen:results.reduce((n,x)=>n+x.freshness.unseen,0),seenRecently:results.reduce((n,x)=>n+x.freshness.seenRecently,0),prioritizedUnseen:results.reduce((n,x)=>n+x.freshness.prioritizedUnseen,0),retainedSeenForCoverage:results.reduce((n,x)=>n+x.freshness.retainedSeenForCoverage,0)},sourceFetchConcurrency:8,articleReadConcurrency:10,clusterReadCap:120,feedItemCap:120,discoveryResultCap:60,providerPolicy:{maxQueriesPerRun:newsIntakeProviderPolicy.maxQueriesPerRun,maxResultsPerQuery:newsIntakeProviderPolicy.maxResultsPerQuery,cacheTtlHours:newsIntakeProviderPolicy.cacheTtlMs/3600000,maxEstimatedCostPerRun:newsIntakeProviderPolicy.maxEstimatedCostPerRun},newsDiscovery:summarizeNewsDiscoveryQueue(newsDiscoveryQueue),coverage:newsCoverage,coverageDrivenQueries:coverageNewsQueue.length,sourceExpansion:summarizeSourceExpansionQueue(sourceExpansionQueue),sourceExpansionQueries:sourceExpansionQueue.length,rotatingQueries:rotatingNewsQueue.length+newsBackfill.length,newsSlots:newsDiscoveryQueue.length,municipalProtocols:summarizeMunicipalProtocolDiscovery(municipalProtocolQueue),municipalProtocolSlots:municipalProtocolQueue.length,competitorJobs:summarizeCompetitorJobDiscovery(competitorJobQueue),competitorJobSlots:competitorJobQueue.length,publicRecords:summarizePublicRecordsDiscovery(publicRecordsQueue),publicRecordSlots:publicRecordsQueue.length,authoritySlots,combinedDiscoveryQueries:combinedDiscoveryQueue.length},authorityDiscoveryCoverage,authorityDiscoveryBatch,authoritySearchQueue,municipalProtocolQueue,competitorJobQueue,competitorJobSignals,competitorWebsiteMonitor,entityGraph,competitorEntityViews,publicRecordsQueue,ambiguousCaseHoldingArea,persistentAmbiguousCandidates,fusedSignals,signalTimelines,signalTimelineSummary,whyItMatters,whyItMattersSummary:whyItMattersSummaryPayload,sourceGaps,sourceGapSummary,gapDrivenDiscoveryPlan,gapDrivenDiscovery,gapDiscoveryFeedbackObserved,gapDiscoveryFeedback,evidencePromotion:{load:pendingEvidenceLoad.status,summary:evidencePromotionBatch.summary,marked:evidencePromotionMark,save:pendingEvidenceSave},leadTimeSummary,caseSnapshots,caseHistoryPersistence,caseEvolutions,caseEvolutionSummary,strategicDeltas,strategicDeltaSummary,evidenceDrilldowns,coolingAssessments,coolingSummary,intelligencePriorities,intelligencePrioritySummary,analystActionQueue,analystActionSummary,newsCoverage,rotatingNewsQueue,coverageNewsQueue,sourceExpansionQueue,newsBackfill,newsDiscoveryQueue,discoveryProvider,discoveryPipeline,activeDiscovery,sourceSuggestions,discoveryResults:[...activeDiscovery.results,...discoveryResults].sort((a,b)=>b.score-a.score).slice(0,60),sourceStatus,persistentIntelligence,
    items,
