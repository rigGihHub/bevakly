...
export async function GET(req:NextRequest){
  ...
  const enriched=await mapWithConcurrency(clusters,10,async group=>{
    ...
    try{const articleHtml=await settleBefore(refreshDeadlines.articles,()=>fetchText(primary.url));if(!articleHtml)throw new Error('article time budget'); article=extractArticle(articleHtml,profile.keywords); if(primary.publishedAtHint)article={...article,title:primary.title,publishedAt:primary.publishedAtHint}; discoveryObservations.push({pageUrl:primary.url,pageSource:primary.source,html:articleHtml}); articleReadOkCount++; articleExtractionMethods[article.extractionMethod]=(articleExtractionMethods[article.extractionMethod]??0)+1; if(article.extractionMethod!=='none'&&article.extractionMethod!=='metadata')articleExtractionRecovered++; if(article.extractedChars<120)articleExtractionThin++; if(article.publishedAt)dateRecoveredFromArticle++;}catch{articleReadOk=false; articleReadFailed++;}
