...
export async function GET(req:NextRequest){
  ...
  const coverageGapPolicy={...newsIntakeProviderPolicy,maxQueriesPerRun:4,maxResultsPerQuery:8,maxEstimatedCostPerRun:0.04};
  const coverageGapDiscovery=await runDiscoveryOrchestrator({
    providers:configuredProviders,
    queue:rankedCoverageGapQueue,
    industryKeywords:profile.keywords,
